...
const ViewerPanel: React.FC<ViewerPanelProps> = ({ status }) => {
  ...
  // State to track last message for confetti trigger
  ...
  // Effect to trigger Confetti on new incoming messages
  useEffect(() => {
    if (messages.length === 0) return;

    const newestMsg = messages[messages.length - 1];

    // Se é a primeira carga (lastProcessedMsgId é null), apenas marcamos o ID para não disparar confete do histórico
    if (lastProcessedMsgId === null) {
        setLastProcessedMsgId(newestMsg.id);
        return;
    }

    // Se detectamos uma nova mensagem
    if (newestMsg.id !== lastProcessedMsgId) {
        setLastProcessedMsgId(newestMsg.id);

        // Se a mensagem é do tipo reação (confete) e NÃO foi enviada por mim (pois eu já disparei no clique)
        if (newestMsg.type === 'reaction' && newestMsg.sender !== username) {
            triggerConfetti();
        }
    }
  }, [messages, username, lastProcessedMsgId]);
  ...
  const triggerConfetti = () => {
      confetti({
        ...
        origin: { y: 0.7 },
        zIndex: 9999,
        colors: ['#EAB308', '#FACC15', '#CA8A04', '#FFFFFF', '#EF4444'] 
      });
  };

  const handleSendConfetti = async () => {
      if (!username) return;

      // Dispara localmente imediatamente para feedback instantâneo
      triggerConfetti();

      // Envia para o chat para que outros vejam
                    ...
                    return (
                        <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} group/msg`}>
                            <div className="flex items-center gap-2 max-w-[90%]">
                                ...
                                {isReaction ? (
                                     <div className={`px-4 py-2 rounded-full text-sm font-bold border ${isMe ? 'bg-gold-500/10 border-gold-500/30 text-gold-400' : 'bg-zinc-800 border-zinc-700 text-zinc-300'}`}>
                                         {msg.sender} {msg.text}
                                     </div>
                                ) : (
            ...
            <form onSubmit={handleSendMessage} className="p-4 bg-zinc-950 border-t border-zinc-800 rounded-b-xl">
                <div className="flex items-center gap-2">
                    <button 
                        ...
                        className="p-3 bg-zinc-900 hover:bg-zinc-800 text-gold-500 border border-zinc-800 hover:border-gold-500/30 rounded-full transition-all active:scale-95"
